...
async function showLanguageSelection(chatId) {
  const menuText = `Пожалуйста, выберите язык / Please choose your language / يرجى اختيار لغتك:
...
// Function to translate store names for display
function translateStoreName(storeName, language) {
  ...
  return storeTranslations[storeName]?.[language] || storeName;
}
...
function getNoPromosMessage(storeName, language) {
  const translatedStoreName = translateStoreName(storeName, language);
  const messages = {
    ru: `😔 К сожалению, активных промокодов для ${translatedStoreName} сейчас нет.\n\n/start - Вернуться к выбору магазинов`,
    en: `😔 Unfortunately, there are no active promo codes for ${translatedStoreName} right now.\n\n/start - Back to store selection`,
    ar: `😔 للأسف، لا توجد رموز خصم نشطة لـ ${translatedStoreName} الآن.\n\n/start - العودة إلى اختيار المتاجر`
  };
  return messages[language] || messages.ru;
}
...
function getPromosHeaderMessage(storeName, language) {
  const translatedStoreName = translateStoreName(storeName, language);
  const messages = {
    ru: `🛍️ Активные промокоды ${translatedStoreName}:\n\n`,
    en: `🛍️ Active promo codes for ${translatedStoreName}:\n\n`,
    ar: `🛍️ رموز الخصم النشطة لـ ${translatedStoreName}:\n\n`
  };
  return messages[language] || messages.ru;
}
...
function getPromoFoundMessage(promo, language) {
  ...
  const currency = language === 'ar' ? 'ريال' : '₽';
  const translatedStoreName = translateStoreName(promo.store, language);
  ...
  const label = labels[language] || labels.ru;
  const copyText = language === 'ru' ? ' - Копировать код' : (language === 'en' ? ' - Copy code' : ' - نسخ الرمز');
  
  return `${label.found}\n\n` +
         `💳 ${promo.code}\n` +
         `🎁 ${promo.discount}\n` +
         `${label.minPrice} ${promo.minPrice}${currency}\n` +
         `${label.store} ${translatedStoreName}\n` +
