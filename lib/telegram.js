...
Просто отправьте название магазина или используйте команды:
/new - Новые промокоды`;
...
async function showNewPromos(chatId) {
  try {
    const sevenDaysAgo = new Date();
    sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);

    const newPromos = await Promo.find({
      isActive: true,
      expiresAt: { $gt: new Date() },
    ...
    if (newPromos.length === 0) {
      await sendMessage(chatId, `😔 Новых промокодов за последние 7 дней не найдено.\n\n/start - Вернуться к выбору магазинов`);
      return;
    }

    let messageText = `🆕 Новые промокоды (за последние 7 дней):\n\n`;
    
    newPromos.forEach((promo, index) => {
      const expiryDate = new Date(promo.expiresAt).toLocaleDateString('ru-RU');
      const createdDate = new Date(promo.createdAt).toLocaleDateString('ru-RU');
      messageText += `💳 ${promo.code}\n`;
      messageText += `🏪 ${promo.store}\n`;
      messageText += `💰 Минимальная сумма: ${promo.minPrice}₽\n`;
      messageText += `📍 Регионы: ${promo.locations.join(', ')}\n`;
      messageText += `📅 Добавлен: ${createdDate}\n`;
      messageText += `⏰ Действует до: ${expiryDate}\n`;
      if (index < newPromos.length - 1) messageText += '\n';
    });
    ...
    console.error('Error showing new promos:', error);
    await sendMessage(chatId, 'Ошибка при получении новых промокодов. Попробуйте позже.');
  }
}
